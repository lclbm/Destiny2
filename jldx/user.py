...
def postMsg(r):
    ...
    if S == 'ConnectError':
        ...
        forminfo = getFormInfo(resurl['url'].split(
            '/')[-2], resurl['url'], resurl['csrf'], S)
        forminfo = makeFormData(resurl['url'], forminfo)
        postmsg(resurl['url'].split('/')[-2], resurl['csrf'], forminfo, S)
        return f'打卡成功,时间:{times}'


def abc():
    r = [["zhanghq1219", "ZHQ20010621", "1401", "6", "1219", "412", "220"],
         ["jiangyuan1119", "jy18973458327", "1401", "3", "1119", "368", "526"]]
    msg = '\n'
    for i in r:
        msg += postMsg(i) +'\n'
    return msg
